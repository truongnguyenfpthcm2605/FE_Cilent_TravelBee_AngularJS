app.controller("loginController", function ($scope, $location, $http, $rootScope, $cookies) {
    ...
    $scope.submitForm = function () {
        if ($scope.formlogin.$valid) {
            $http.post($rootScope.url + "/api/v1/auth/login", $scope.login)
                .then(response => {
    ...
    $rootScope.social = function (provider) {
        $http.get($rootScope.url + provider)
            .then(response => {
                $scope.object = response.data
                $rootScope.email = $scope.object.email
                $rootScope.fullname = $scope.object.fullname
    ...
    $scope.onRememberClick = function () {
        if ($scope.rememberMe) {
            ...
            $cookies.put('userInfo', JSON.stringify($scope.login), { expires: expirationDate });
        } else {
